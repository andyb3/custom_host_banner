...
function addBanner(data) {
  if (location.host in data.hosts) {
    ...
    if (data.hosts[location.host]["display"] === "insert") {
      insert_banner(document.getElementById("host-banner"));
    } else if (data.hosts[location.host]["display"] === "on_top") {
      overlay_banner(document.getElementById("host-banner"));
    }
  }
}
...
function insert_banner(banner) {
  ...
  // Add the sticky class to the header when you reach its scroll position. Remove "sticky-banner" when you leave the scroll position
